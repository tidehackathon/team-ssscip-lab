...
def get_url(text):
    ...
    try:
        ...
        r = s.lookup_urls([url])
        if r[f'{url}']['malicious'] == False:
            return -1
        elif r[f'{url}']['malicious'] == True:
            return 1
    except:
        return 0
...
def engagment_rate(tweet):
    ...
    try:
        engagement_rate = (likes + retweets) / tweet.user.followers_count
    except:
        engagement_rate = (likes + retweets) / 1
...
def calculate_informality(text):
    ...
    for tag in tags:
        ...
        if pos == 'DT' or pos == 'IN' or pos == 'TO' or pos == 'PRP' or pos == 'MD' or pos == 'RB':
            informal_count += 1
    try:
        informality = informal_count / word_count
    except:
        informality = informal_count / 1
    return informality
...
while True:
    ...
    print(titles)
    for i in titles:
        func(num_tweets=30,search_query=i)
